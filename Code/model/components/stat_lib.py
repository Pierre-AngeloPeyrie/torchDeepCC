...
class InverseGamma:
    ...
    def eval(self, x):
        t1 = torch.pow(x, -self.alpha-1)
        t2 = torch.exp(-self.beta / x)
        res = (self.a1 / self.a2) * t1 * t2
        energy = torch.sum(- torch.sum(torch.log(res+1e-12), dim=1))
        return res, energy
...
class GaussianMixtureModeling:
    def __init__(self, gmm_config):
        ...
        # self.mixture_cov_0 = torch.from_numpy(np.ones([self.num_mixture, 1])*0.1, dtype=torch.float32)
    ...
    def eval(self, x, p):
        ...
        phi = torch.mean(p, dim=0)
        # Augmenting input: [batch_size, num_mixture, num_dim]
        x_t = torch.reshape(x, shape=[-1, 1, self.num_dim])
        x_t = torch.tile(x_t, [1, self.num_mixture, 1])
        # mixture mean: [num_mixture, num_dim]
        p_t = torch.reshape(p, shape=[-1, self.num_mixture, 1])
        z_p = torch.sum(p_t, dim=0)
        mixture_mean = torch.sum(x_t * p_t, dim=0) / z_p
        # mixture diagonal covariance: [num_mixture, num_dim]
        z_t = (x_t - mixture_mean) ** 2
        # mixture_cov_1 = torch.sum(z_t * p_t, dim=0)[0:1, 2:1]
        # print mixture_cov_1
        # mixture_cov_1 = mixture_cov_1 / z_p
        # print mixture_cov_1, z_p
        # mixture_cov = torch.concat([self.mixture_cov_0, mixture_cov_1], 1)
        # mixture_cov =torch.sum(z_t * p_t, 0) / z_p
        mixture_cov = np.ones([self.num_mixture, 1]) * 0.001
        mixture_dev = mixture_cov ** 0.5
        # probability density evaluation
        z_norm = torch.sum(z_t / mixture_cov, dim=2)
        mixture_dev_det = torch.prod(mixture_dev, dim=1)
        t1 = torch.exp(-0.5 * z_norm)
        ...
        pen_dev = torch.sum(1.0 / mixture_cov[:, 0:self.num_dynamic_dim])
        # pen_dev = torch.sum(1.0 / mixture_cov)
        # pen_dev = torch.sum(torch.sum(- torch.log(mixture_cov), dim=1) * phi)
    ...
    def test(self, x, phi, mixture_mean, mixture_dev, mixture_cov):
        ...
        z_t = (x_t - mixture_mean) ** 2
        # probability density evaluation
        z_norm = torch.sum(z_t / mixture_cov, 2)
        mixture_dev_det = torch.prod(mixture_dev, dim=1)
        t1 = torch.exp(-0.5 * z_norm)
        t2 = ((2 * math.pi) ** (0.5 * self.num_dim)) * mixture_dev_det
        # Likelihood
    ...
    def vi_learning(self, x, p):
        ...
        x_t = torch.reshape(x, shape=[-1, 1, self.num_dim]) # self.num_dim, self.num_mixture, or XXX
        #x_t = torch.reshape(x, shape=[-1, 1, 40])
        x_t = torch.tile(x_t, [1, self.num_mixture, 1])
        ...
        mixture_cov = torch.sum(z_t * p_t, dim=0) / z_p #[num_mixture, num_dim]
        #mixture_cov = torch.ones([self.num_mixture, self.num_dim], torch.float64) * 1
        mixture_dev = mixture_cov ** 0.5
        
        # probability density evaluation
        z_norm = torch.sum(z_t / mixture_cov, dim=2)
        mixture_dev_det = torch.prod(mixture_dev, dim=1) #[num_mixture]
        t1 = torch.exp(-0.5 * z_norm)
        t2 = ((2 * math.pi) ** (0.5 * self.num_dim)) * mixture_dev_det
...
class SoftKMeansMixtureModeling:
    ...
    def eval(self, x, p):
        ...
        # mixture diagonal covariance: [num_mixture, num_dim]
        z_t = (x_t - mixture_mean) ** 2
        # mixture_cov = torch.sum(z_t * p_t, dim=0) / z_p
        # mixture_cov = torch.maximum(mixture_cov, 1e-2)
        # mixture_dev = mixture_cov ** 0.5
        # probability density evaluation
        # z_norm = torch.sum(z_t / (mixture_cov + 1e-12), dim=2)
        # mixture_dev_det = torch.prod(mixture_dev, 1)
        # t1 = torch.exp(-0.5 * z_norm)
        # t2 = ((2 * math.pi) ** (0.5 * self.num_dim)) * mixture_dev_det
        # # Likelihood
        # energy = torch.mean(-torch.log(torch.sum(phi * t1 / t2, dim=1)))
        distance = torch.mean(torch.sum(torch.sum(z_t, 2) * phi, dim=1))
        # pen_dev = torch.sum(0.05 / mixture_cov)
        return distance
